    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                -- Unique per-RUN tag: controller_id is stable across runs, so
                -- order/executor counts must be scoped by the run's session id
                -- or one run's stats bleed into the next.
